...
def test_generate_rag_answer_gemini_failure_fallback_to_ollama():
    ...
    with patch("app.core.rag.llm_provider.call_gemini_api", side_effect=httpx.HTTPStatusError("429 Too Many Requests", request=None, response=None)):
...
def test_generate_rag_answer_all_llms_fail_fallback_to_extractive():
    candidates = [
