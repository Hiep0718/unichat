...
def generate_rag_answer(
    ...
    if allow_external_knowledge:
        system_prompt = (
            ...
            "4. ĐỊNH DẠNG VĂN BẢN: "
            "Viết text bình thường bằng đoạn văn, heading (#, ##, ###), danh sách (-, 1.) và **in đậm**. "
            ...
            "6. SƠ ĐỒ TRỰC QUAN (MERMAID): Khi câu trả lời liên quan đến quy trình, phân cấp, so sánh, quan hệ hoặc luồng nghiệp vụ, "
            "BẮT BUỘC tạo ít nhất 1 sơ đồ Mermaid bằng khối code ```mermaid. "
            "QUY TẮC BẮT BUỘC KHI VIẾT MERMAID:\n"
            "   - KHÔNG bao giờ đặt ký hiệu trích dẫn [1], [2] bên trong node sơ đồ.\n"
            "   - Node label chỉ dùng chữ cái, số và khoảng trắng. KHÔNG dùng dấu hai chấm ':', dấu gạch ngang '-', ngoặc vuông, ngoặc nhọn.\n"
            "   - Nếu label cần ký tự đặc biệt, bọc trong dấu ngoặc kép: \"Label co dau dac biet\".\n"
            "   - Với mindmap dùng thụt lề 2 khoảng trắng mỗi cấp.\n"
            "   Ví dụ đúng:\n"
            "   ```mermaid\n"
            "   mindmap\n"
            "     root((Chu de chinh))\n"
            ...
            "         Chi tiet B1\n"
            "   ```\n"
            "   Các loại sơ đồ: mindmap, flowchart TD, sequenceDiagram, classDiagram, gantt, pie.\n"
        ...
        system_prompt = (
            ...
            "6. SƠ ĐỒ TRỰC QUAN (MERMAID): Khi câu trả lời liên quan đến quy trình, phân cấp, so sánh, quan hệ hoặc luồng nghiệp vụ có trong tài liệu, "
            "hãy tạo sơ đồ Mermaid bằng khối code ```mermaid để minh họa trực quan. "
            "Chỉ dùng thông tin CÓ TRONG tài liệu để vẽ sơ đồ. "
            "KHÔNG đặt [1], [2] trong node sơ đồ. Node label chỉ dùng chữ cái, số, khoảng trắng. "
            "Nếu cần ký tự đặc biệt, bọc label trong ngoặc kép.\n"
            "7. LUẬT BẮT BUỘC Ở CUỐI CÂU TRẢ LỜI: Ngay sau khi hoàn thành toàn bộ câu trả lời, "
            "bạn BẮT BUỘC phải tạo một đường phân cách '\\n\\n---\\n\\n' và mỗi gợi ý BẮT BUỘC phải ở một dòng riêng bắt đầu bằng dấu gạch ngang '- ' như sau:\n\n"
            "---\n\n"
            "### 💡 Gợi ý câu hỏi & bước tiếp theo:\n"
