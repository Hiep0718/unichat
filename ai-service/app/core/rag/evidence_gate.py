...
class EvidenceGateResult:
    def __init__(self, decision: DecisionEnum, evidence_score: float, refusal_reason: str | None = None):
...
def evaluate_evidence(
    ...
    if intent == IntentEnum.OUT_OF_SCOPE:
        return EvidenceGateResult(DecisionEnum.REFUSE, 0.0, "Câu hỏi nằm ngoài phạm vi hỗ trợ của hệ thống.")

    if not candidates:
        return EvidenceGateResult(DecisionEnum.REFUSE, 0.0, "Tài liệu hiện có chưa đủ bằng chứng để trả lời câu hỏi này.")
    ...
    if num_source_groups < strategy.min_source_groups:
        return EvidenceGateResult(
            DecisionEnum.REFUSE,
            0.0,
            f"Thiếu nguồn thông tin đối sánh (yêu cầu tối thiểu {strategy.min_source_groups} nguồn tài liệu).",
        )
